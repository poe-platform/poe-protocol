...
class PoeServer():
        ...
        while True:
            ...
            if answer == "!q":
                if self.bot_client and self.bot_client.session:
                    asyncio.get_event_loop().run_until_complete(self.bot_client.session.close())
                return
            elif answer == "!c":
                ...
                print_formatted_text(HTML(f'<info>Debug set to {self.debug}</info>'), style=style)
                continue
    ...
    async def send_message(self, msg: str):
        ...
        content = ""
        async for event in self.bot_client.stream_request(msg, self.context, debug=self.debug):
            if event.message == 'text':
                text_data = json.loads(event.data)
                content += text_data["text"]
                print(text_data["text"])
            elif event.message == 'done':
                self.context.messages.append(self.bot_client.build_single_Message("bot", content))
                return
